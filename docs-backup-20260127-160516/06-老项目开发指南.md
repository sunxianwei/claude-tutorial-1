...
> 💡 **快速上手？** 如果你只需要快速接入，可以直接跳到 [⚡ 10分钟快速接入](#10分钟快速接入) 部分。

## 目录

1. [⚡ 10分钟快速接入](#10分钟快速接入)（新增）
2. [为什么老项目需要特殊处理](#为什么老项目需要特殊处理)
3. [准备工作](#准备工作)
4. [项目分析阶段](#项目分析阶段)
5. [创建项目规范文件](#创建项目规范文件)
6. [渐进式接入策略](#渐进式接入策略)
7. [常见场景和 Prompt](#常见场景和-prompt)
8. [实战案例](#实战案例)（新增）
9. [最佳实践](#最佳实践)
10. [常见问题](#常见问题)
...
如果你想快速体验 Claude Code 在老项目中的效果，按以下步骤操作：
...
```bash
# 让 Claude 生成 CLAUDE.md
> 根据现有代码推断项目的编码规范，并生成 CLAUDE.md

# 添加 .claudeignore
> 创建 .claudeignore，排除 node_modules、build、dist 等目录
```
...
1. **初始化**：`cd project && claude .`
2. **分析项目**：让 Claude 分析结构、技术栈、核心功能
3. **创建摘要**：生成项目概况文档
...
### 案例 1：Spring Boot 2.x 升级到 3.x

**背景**：老项目使用 Spring Boot 2.5，需要升级到 3.x 以获得新特性和安全更新。
...
## 必需改动
1. **Java 版本**：最低 Java 17（当前项目使用 Java 11）
2. **包名变更**：javax.* → jakarta.*
3. **移除的类**：WebSecurityConfigurerAdapter

## 依赖变化
- spring-boot-starter-parent: 2.5.x → 3.2.x
- spring-cloud-dependencies: 2021.0.x → 2023.0.x

## 配置变化
- spring.jpa.hibernate.ddl-auto 默认值改变
- spring.security 配置方式改变
